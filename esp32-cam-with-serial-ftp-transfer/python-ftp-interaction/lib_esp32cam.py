...
CHUNK_SIZE = 2048
TRANSFER_BAUDRATE=921600
# TRANSFER_BAUDRATE=2_000_000
...
class ESP32CamSerial:
    ...
    def connect(self) -> bool:
        ...
        if self._try_baud(TRANSFER_BAUDRATE):
            ...
            while time.time() - start < self.timeout:
                if self.ser.in_waiting:
                    ...
                    if b"Send 'MODE TRANSFER' to switch" in response:
                        ...
                        time.sleep(0.5)
                        return self._try_baud(TRANSFER_BAUDRATE)
                time.sleep(0.1)
    ...
    def store_file(self, path: str, data: bytes) -> bool:
        ...
        # Send STORE directly — do NOT use _send_cmd which waits for the prompt.
        # The ESP32 storeFile() sends "Ready" then waits for ACK mid-command,
        # so the prompt is not available until after the full transfer.
        self._log("  Sending STORE command...")
        self.ser.write(f"STORE {path} {size}\n".encode())
        self.ser.flush()

        # Read the "Ready" response before the ESP32 waitForAck() times out (5s).
        response = self._read_line(timeout=10)
        self._log(f"  Pre-upload response: {response[:150]!r}")

        if "[OK] Ready" not in response:
            if "[ERROR]" in response:
