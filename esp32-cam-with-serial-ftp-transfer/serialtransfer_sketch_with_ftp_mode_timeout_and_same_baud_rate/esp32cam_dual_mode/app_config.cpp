...
static bool parseConfigLine(const String& line, Config& cfg) {
  int eq = line.indexOf('=');
  ...
  if (key.equalsIgnoreCase("interval")) {
    ...
    cfg.mode = stringToDeviceMode(val);
  } else {
    return false;
  }
  return true;
...
bool writeConfig(fs::FS& fs, const Config& config) {
  ...
  f.println("# mode: PHOTO or TRANSFER");
  f.println();
  ...
  f.printf("mode=%s\n", deviceModeToString(config.mode).c_str());
  f.close();

  Serial.println("[INFO] Config written to " CONFIG_PATH);
  return true;
}
...
String deviceModeToString(DeviceMode m) {
  return (m == MODE_TRANSFER) ? "TRANSFER" : "PHOTO";
}

DeviceMode stringToDeviceMode(const String& s) {
  if (s.equalsIgnoreCase("TRANSFER")) return MODE_TRANSFER;
  return MODE_PHOTO;
}
